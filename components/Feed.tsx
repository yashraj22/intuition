...
const Feed = () => {
  ...
  useEffect(() => {
    const fetchInitialFeed = async () => {
      ...
      try {
        const response = await fetch("/api/get-feed");
        if (!response.ok)
          throw new Error("Failed to fetch initial source items");
        const data = await response.json();
        setItems(data.data);
      } catch (error) {
        console.error("Error fetching initial source items:", error);
  ...
  useEffect(() => {
    if (!loading && items.length > 0) {
      // Only schedule the next update after initial load is complete
      const timeoutId = setTimeout(() => {
        ...
        fetchSources(feedsToFetch);
      }, 10000); // Wait 10 seconds after initial load before fetching updates
  ...
  const parseHtmlContent = (html: string): string => {
    // Use a lightweight approach instead of full DOM parsing
